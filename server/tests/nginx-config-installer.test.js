...
function fixture() {
  ...
  for (const directory of [targetDir, backupRoot, runRoot, bin]) fs.mkdirSync(directory, { recursive: true })
  fs.chmodSync(targetDir, 0o755)
  fs.chmodSync(backupRoot, 0o700)
  write(target, oldConfig, 0o600)
...
if [[ "$1" == exec ]]; then
  container="$2"
  shift 2
  if [[ "$container" == nginx && "$1" == rm ]]; then rm -f '${containerCandidate}'; exit 0; fi
...
async function run() {
  ...
  try {
    const result = execute(locked, locked.candidate, undefined, { LOCK_FAIL: '1' })
