...
function fixture({ missingOverlay = false, tamperOverlay = false, missingIp = false, renderMismatch = false, oldCompose = false } = {}) {
  ...
  write(contract, `#!/usr/bin/env bash
set -euo pipefail
[[ "$1" == validate-network-config && "$2" == '${networkConfig}' ]]
grep -qx 'KINVEST_WEB_NETWORK=web' "$2"
grep -qx 'KINVEST_NGINX_CONTAINER=nginx' "$2"
grep -qx 'KINVEST_NGINX_IPV4=172.19.0.9' "$2"
printf '%s\n' '{"container":"nginx","network":"web","ip":"172.19.0.9","trustedProxyAddresses":"[\\"172.19.0.9\\"]","checksum":"${'a'.repeat(64)}"}'
`, 0o755)
  write(path.join(bin, 'id'), '#!/bin/sh\n[ "${1:-}" = -u ] && { echo 0; exit 0; }\nexec /usr/bin/id "$@"\n', 0o755)
  write(path.join(bin, 'docker'), `#!/usr/bin/env bash
set -euo pipefail
printf 'docker:%s:ip=%s\n' "$*" "\${KINVEST_NGINX_IPV4:-missing}" >>'${operations}'
...
function execute(context, mode, extraEnv = {}) {
  return spawnSync('bash', [context.gate, mode], {
    encoding: 'utf8',
    env: { ...process.env, PATH: `${context.bin}:${process.env.PATH}`, ...extraEnv }
  })
}
...
async function run() {
  ...
  try {
    ...
    assert.doesNotMatch(operations, / up |198\.51\.100\.77/)
